...
- **The maintainer report now sizes the next feature, not just the last one.** `/build-report-maintainer` has an **Effort benchmarks** section: how long and how much each kind of screen actually took to build (list, form, detail, record action, sign-in, export), the same figures rolled up per feature, and a calculator you type a screen mix into to size the next piece of work against your own delivered history. It gives both the marginal cost of one more screen and the fully-loaded cost once planning, end-of-feature testing and review are counted in, and it says when features were built in parallel — their story windows overlap, so the per-feature split is approximate while the totals still hold. It uses the timings and token records the workflow already keeps, so there's nothing to switch on. Where the logs can't support a per-story cost it shows build times only and says why, rather than quoting a figure it can't stand behind.
...
- **All reports now land in one folder: `generated-docs/reports/`.** Every report page and the data behind it is written there — the maintainer retrospective and the client-facing delivery report — so they sit together instead of being scattered through `generated-docs/` alongside your project's own documents. Each report tells you its path when it finishes, and the reports remain excluded from version control since any of them can be rebuilt at any time. If you're upgrading an existing project, report pages from earlier versions stay where they are but are left out of version control too, so they won't clutter anything; you can delete `generated-docs/build-report*` by hand if you'd like them gone. The team name shown on the maintainer report is asked for once more, the first time you run it.

- **Two reports, split by who's reading them — and the cost detail is now part of the maintainer one.** `/build-report-maintainer` carries everything internal on one page: effort and cost, workflow performance, the effort benchmarks and sizing calculator, and a **Spend detail** fold holding the per-model, per-tool and per-helper breakdowns plus the verbatim log of every question you were asked and the answer you gave. `/build-report-stakeholders` stays a separate page because it works by leaving cost and internal machinery out, which can't safely be a section you collapse. Both are listed in the command reference. **`/workflow-insights` is gone** — everything it showed is in that Spend detail fold, and `/upgrade` removes the retired command for you. One page also means one set of figures: separate reports each refreshed the cost data themselves and asked you for the exchange rate again, so two of them could end up disagreeing about the same project.
...
- **A feature that's only part-way through is no longer reported as a small feature.** The per-feature figures come from each story's recorded start and finish times, so a feature still being built only has times for the stories it has finished. One finished story used to be treated as enough to measure the whole thing — so a feature with two of its eight stories done was reported as if two stories were all there was, and the rest of its build time disappeared from the page. Those features now keep the separate time estimate the report already had for them, are marked with how much of them was actually measured, and are kept out of the "typical feature" figures — so the calculator you size your next piece of work with isn't dragged down by a half-measured feature.

- **A report for one project no longer counts another project's work.** If you keep several copies of a project side by side whose folder names start the same way — `my-app`, `my-app-QA`, `my-app-copy` — the report for `my-app` was folding the others' recorded work into its totals. That work sits outside any story, so it inflated the overhead figure the report multiplies costs by, and the "fully loaded" costs and the sizing calculator came out high as a result. The report now checks with git which folders really belong to the same project, so the folders `/plan` creates still count and unrelated copies don't.

- **A report is no longer lost because one figure is missing from its data.** An effort data file left over from an older version, or copied from another project, could stop the whole page from being produced. Any figure it can't read now shows as a dash in that one spot and the rest of the report is generated as normal.

- **Report pages from before the reports were merged are now cleaned up.** The two standalone pages that became sections of the maintainer report stopped updating when they were folded in, but were left on disk — so a bookmarked one looked current while showing figures that disagreed with the report that replaced it. Generating a report now removes them.

- **`/upgrade` removes the three retired report commands.** `/build-report-cost`, `/build-report-effort` and `/build-report-all` are gone, but on a project too old to record which version it came from they were being left in place — and each still ran, overwriting the data the current report reads. They're now removed like every other retired command, along with the empty folders they leave behind.

- **"There's no project to report on yet" is told apart from "one detail is missing".** The first stops with a pointer to `/start`, since every later step would fail the same way; the second still produces the report and names the gap in its Data quality section. The two were previously distinguished by matching the wording of an error message, which had drifted out of step, so the first case could be mistaken for the second.
