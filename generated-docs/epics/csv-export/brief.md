...
| BR2 | If the current narrowing matches zero requests, the export action does not produce an empty/junk file — surface the existing empty-state message ("No expense requests match the current search and filters.", per `documentation/requirements-application.md` §5 Flow: Export transactions) instead of triggering a download. |
