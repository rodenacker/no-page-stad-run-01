...
- Notifications use the existing root-layout `ToastProvider` / `ToastContainer` and `useToast()` from `web/src/contexts/ToastContext.tsx`; its default 5s auto-dismiss already sits inside R11's 4–8s window, so no new timer mechanism is needed. **A message the user must acknowledge must not use the auto-dismissing toast.**
...
- **No self-approval guard** (R5/BR2) — an Approver deciding their own expense is expected behaviour, deliberately allowed.
