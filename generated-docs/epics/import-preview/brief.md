...
- **BR3 — Compliance split between the two halves (SETTLED — user-confirmed 2026-08-17).** Will-import rows in the preview are treated as listed expense payment requests, so `AccountNumber` is masked to its last four digits per project.md's POPIA rule, exactly as `expense-request-list` already does — applied by this epic at render time to file-sourced values, using `lastFourDigitsOf` (`lib/transactions/display.ts`) as the single source of truth for how masking is done, the same as everywhere else in this project. Rejected rows follow `file-validation-and-retry`'s existing precedent of showing a rejected row's own values **verbatim**, including the full account number, because the purpose of that view is to show the user what their file actually contains so they can fix it. Both rules are correct and both already exist elsewhere in this project; the user confirmed showing both conventions side by side, clearly labelled, on this one screen.
...
- **BR5 — Correction-CSV column shape is an open question for BUILD (flag, do not silently pick).** The correction CSV must carry exactly the seven upload columns, unmasked, in the upload's own order, so it round-trips through `POST /v1/files/upload` untouched. Whether to add an eighth, trailing column carrying the row's rejection reason is **not resolved here**: it would help the employee correcting the row, but it would also break a straight re-upload unless the transactions-api ignores unknown columns (undocumented — not verified against `transactions-api.yaml`, which describes no upload-side column validation at all). State this trade-off to the user at story planning; do not decide it unilaterally during BUILD.
...
- **Correction-CSV trailing reason column — open question, not decided here (BR5).** Do not add a reason column to the correction CSV without confirming the transactions-api tolerates unknown upload columns; the spec is silent on this. If unconfirmed, ship without the trailing column (guarantees a clean round-trip) and offer the reason only in the on-screen preview.
