...
## Story 7 — Backend addresses resolved at request time

- The app now looks up its two backend addresses while it is running, instead of having them fixed in place when it is built. Nothing changes on screen. It means one build of the app can be deployed to every environment — test, staging, live — instead of needing a separate build for each, and it makes the automated browser tests pass against a real production build rather than only against the development server.
