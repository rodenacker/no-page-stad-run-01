...
## Epic end — browser tests

- The browser tests found two real problems, both in shared building blocks rather than in the request screen itself, and both were fixed:
  - **Picking a filter value briefly blanked the page for a screen reader.** The drop-down faded as it closed, and for that fraction of a second everything except the drop-down was hidden from assistive technology — even though the filtered list was already on screen. The drop-down now closes instantly instead of fading, so what you can see is always what a screen reader can read. This affected every drop-down in the app, not just this screen.
  - **The "…" menu on each request kept its name on screen after the mouse left.** The name now appears and disappears with the pointer, and still appears when you Tab to the control.
- Manual test: **passed** on 2026-08-05. All 15 "check these first" backend assumptions and 39 hands-on checks were accepted.
- One planning signal recorded for next time: this epic's manual-test page ran to 54 items, well over the ~30 the workflow expects for a single epic. It is a sign the epic was on the large side and could have been split — worth remembering when planning the remaining four.
- Worth knowing: the masking check (the one that proves account numbers stay hidden while you search, filter, sort and page) failed on its first attempt and passed on the retry after the fix. The fix narrowed the timing window that caused it but has not provably closed it, so this test may occasionally need a second attempt in future runs. It is a timing artefact in how the test reads the screen, not evidence that masking fails — but it is recorded here rather than written off, because the check it performs is a compliance one.
