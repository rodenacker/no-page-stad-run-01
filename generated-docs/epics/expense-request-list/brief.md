...
7. **R7** — Filters narrow the listed requests, and the active filters stay visibly indicated while applied. The filter set is: **status** (one value), **originating file** (one value), **transaction type** (one value), **amount range** (lower bound, upper bound, or both) and **transaction date range** (earliest, latest, or both). The pick-one filters offer the values present in the fetched set, with transaction type shown under the app's plain-language label where it has one and the service's raw value where it does not (R1 — the service owns the accepted set). Both ranges are inclusive of their bounds; amount is compared numerically against `Amount`, date chronologically against `TransactionDate` as the service writes it, with an upper day bound covering a value that carries a time of day. Filters combine: several applied together narrow cumulatively, and clearing the narrowing (R10/R18) clears all of them at once. A range whose upper bound is below its lower bound is reported in place, is not applied, and does not count as active narrowing.

    `[USER-DIRECTED]` The transaction-type, amount-range and date-range filters were added by the user at the `expense-request-list` stories approval (2026-08-05), extending the original status + originating-file pair. Not present in `documentation/requirements-application.md`.
...
- **Design decision resolved at the stories approval (2026-08-05): a request's detail opens in a panel/dialog over the list**, one request at a time, with the reveal-account-number control inside it — chosen over an expandable row. Closing it returns the user to the list with place, sort and page unchanged. The alternative (inline row expansion, several openable at once) was rejected. See `stories/story-5-request-detail-and-account-number.md`.
- **Search scope decided at the stories approval (2026-08-05):** the free-text search (R2/R6) covers what is on screen — `Reference`, `Description`, `FileName`, `Amount`, and the **visible last four digits** of `AccountNumber`. Full account numbers are deliberately **not** searchable: matching against the unmasked value would be a way around the masking POPIA requires, since a searcher could confirm a guessed number without ever revealing it. Do not widen search to the unmasked value in any later epic.
- **The date-format assumption now has a second failure mode.** R7's date range compares `TransactionDate` as the service writes it, so inconsistent formatting — or a time of day on some rows only — can silently exclude requests that really do fall inside the range, with a request dated on the **last day** of the range the most likely casualty. This joins the existing ordering and duplicate-matching exposure; it is carried in `state.json.epic.unverifiedAssumptions` for the manual-test approval. Do not normalise on speculation.
