...
- Neither brief said what "the batch does not balance" actually looks like, so this story settled it and it is written into the epic brief: while a decision is waiting to be confirmed, only the outstanding count states the projected figure — RECORDS and DECIDED go on stating what the batch really is, so the three visibly do not add up. Moving DECIDED with it would quietly re-balance the block and leave nothing for the reader to see.
- The gap is named rather than left to be inferred: a NOT YET CONFIRMED figure joins the control block while something is pending and is absent otherwise (the same rule the selection count follows — an indicator permanently reading 0 is a fixture, not an answer), and every affected row carries the same words beside its status, at phone width as well as on the wide listing.
- The count rolls only when a decision has actually been recorded, never when the projection appears or is backed out of. Otherwise confirming one approval would have produced three separate movements — into the projection, back out of it, then down — where the design asks for one.
- The pre-commit row mark shares the possible-duplicate mark's warning ink and shape, because both are "look at this now". What tells them apart is their words, which is how every mark on this listing is distinguished.

## Epic end: quality, review and the two calls that came out of it

- The epic-end security gate failed on two errors that were not errors: it had matched the words `dangerouslySetInnerHTML` inside a comment explaining why the file's one raw-markup write is safe. The file's two real uses were both already reviewed and accepted. Because the accept-marker attaches to a line of code, there was no way for the author to clear a finding against prose — the only escapes were deleting the explanation or rewording it to dodge the search, which is how a check ends up training people to leave hazards undocumented. The checker was fixed instead, and the wider problem written up for the template's maintainers.
