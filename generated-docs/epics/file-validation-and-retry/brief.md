...
- **`/upload/file` is an async SERVER component.** `web/src/app/(authenticated)/upload/file/page.tsx` calls `requireSession()` + `canAccess()` and returns `<PermissionDeniedMessage deniedPath={…} />` before rendering anything when access is refused. It does not become a client component — the epic's server-side gating rule (source UI-24: role-conditional actions are decided on the server and omitted from the markup, never rendered disabled) depends on it. It hands the identifier down to client units:
  ...
  - `components/files/RejectedRows` — `{ file: FileLog }`; renders nothing at all unless the status is `Validation failed`.
    - **Correction found at epic-end review:** the rejected rows need the same `refreshSignal?: number` the processing history takes, so the full shape is `{ file: FileLog; refreshSignal?: number }`. A retry that fails AGAIN leaves the file in the status it already had, so nothing about the file changes and this section neither unmounts nor re-reads — the previous attempt's rows would stay on screen indefinitely, against FR4 and Key Workflows step 5. For the same reason an accepted retry keeps the file page's single interval running for a fixed number of further reads rather than deciding the outcome from one sample.
...
- **`SubmittedFilesList` keeps its existing prop contract.** `expense-file-upload` story 3 pinned that this component takes no session/role prop and its notification is not role-gated. Story 5 adds **`viewerRoles?: string[]`** — optional, and when absent the component **still notifies**. Present → notify only when it includes `ROLE_IMPORTER`. The `/upload` page always supplies it, from `rolesOf(session)`.
  - **Correction found during BUILD:** the "still notifies when absent" default does *not* leave `expense-file-upload` story 3's tests untouched, as this bullet originally claimed. That story's AC-3 test asserted the notification surface was **not rendered at all** when a file reached `Validation failed` — true only for as long as `R91` was unimplemented, and explicitly justified in that test as "the next epic's R91 must not be pre-empted here". Now that FR9 implements it, that one assertion was narrowed to this story's own criterion (nothing on the screen reports an import for a file that failed); everything about the rejected-rows notification is covered by story 5's own tests.
