...
- The status chip is **already extracted** — `web/src/components/status/StatusBadge.tsx` (built during `expense-request-list`) owns the five intents, their tokens and the neutral/iconless pass-through of an unrecognised status. Import it; do not extract it again and do not declare a second intent→token map. What is still private is the *file* vocabulary: `STATUS_PRESENTATION` (`Record<FileStatus, StatusPresentation>`) and its `FileStatusBadge` wrapper are local to `SubmittedFilesList.tsx`. The file page needs the same chip, so **lift those two out** into a shared module (`web/src/components/files/FileStatusBadge.tsx`) and have the list import it from there — one file-status map in the project, not two. A status must never be shown by colour alone, and no component may name a colour (all tokens live in `web/src/app/globals.css`).
