...
- Two test files disagreed about this one (see below). The earlier epic's test said a file failing validation must raise no notification at all — which was only true while this epic's requirement was still unbuilt, and that test said so itself, pointing at this requirement as the next epic's job. Now that it is built, that one assertion was narrowed to what its own story actually promises: nothing on the screen claims a failed file imported. Everything about the new notification is covered by this story's own tests, and all of the earlier epic's tests still pass.

## Epic-end code review

- Fixed a real gap in the retry flow: if you retried a failed file and it failed again, the page kept showing the previous attempt's rejected rows indefinitely, with no way to refresh them. The page now keeps checking for a short while after any retry it asked for — not just when the file looks busy — and the rejected-row list is refreshed along with the file and its processing history, all driven by the one timer the page already had.
- Fixed the two downloads interfering with each other. Both controls stay usable while a file is on its way (deliberate — disabling a control under a keyboard user's focus is worse), so both files can be on their way at once; previously whichever answered first wiped the other's "preparing…" message, and a refusal on one could be overwritten by the other. Each download now keeps its own progress message and its own failure message, each naming which file it is about.
- Added the two tests that would have caught these: a retry that fails again (asserting the rows shown are the new attempt's, and that a failed refresh leaves the screen alone), and two overlapping downloads where one is refused while the other is still out.
- Hardened the rejected-rows reason lookup so an unexpected field name in the service's response cannot crash the page, and collapsed seven near-identical copies of the "use the service's own wording" helper onto one.
